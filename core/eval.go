...
func evalSet(args []string, connection io.ReadWriter) error {
	...
	for i :=2; i<len(args);i++ {
		...
		default: return  errors.New("(error) ERR syntax error")
		}
	}
...
func EvalAndRespond(command *RedisCmd, connection io.ReadWriter) error {
	...
	case "SET": return evalSet(command.Args, connection)
	case "GET": return  evalGet(command.Args, connection)
	case "TTL": return  evalTtl(command.Args, connection)
